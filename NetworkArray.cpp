	...
	m_geometry(geometry)
{
	...
	NetworkLetter** m_tablo_net = new NetworkLetter*[length_alphabet];

	for (int i = 0; i < m_length_alphabet; ++i)
		m_tablo_net[i] = new NetworkLetter(CHARS[i], m_geometry);
...
void NetworkArray::learn()
{
	...
	tabloFichiers = getArrayOfFileNames();						//on récupère les noms des ficheirs d'exemples
	inputs = getArrayOfExemples(tabloFichiers, nb_exemples);	//on récupère les donnees des exemples
...
char NetworkArray::testNetworks(vd input, bool verbose)
{
	//initialisation
	...
	{											//on récupère la réponse
		exp_output.push_back(m_tablo_net[i]->getOutput(input));
...
double NetworkArray::testAll(string directory)
{
	...
	//Récupération des données des fichiers
	vc tabloFichiers = getArrayOfFileNames(directory);

	//On compte le nombre de succes
	cout << "Test en cours de " << nb_exemples << " fichiers ..." << endl;
	for (int i = 0; i < nb_exemples; ++i)
	{
		vd input = readExemple(tabloFichiers[i],  FIRST_LAYER_SIZE, directory);	//on lit l'exemple
...
void NetworkArray::setGeometry(string geometry)
{
	m_geometry = geometry;
}
...
vector<double> readExemple(char* nom_fichier, int taille_entree, string directory)
{
	...
	else
	{
		...
		for (int i(0); i < taille_entree; i++)
			file >> res[i];	// on insère les valeurs dans entrees
		return res;
	}
}
...
vector<char*> getArrayOfFileNames(string directory)
{
	int				compteur(0);
	DIR*			dp;
	struct dirent*	ep;
	...
	if (dp != NULL)												//Si l'ouverture du répertoire a fonctionné
	{
		while ((ep = readdir(dp)))								//On parcourt le répertoire
		{
			if (strlen(ep->d_name) >= 4)						//Si le nom du fichier est d'au moins 4 caractères du genre .png ou .bmp (sinon le programme détecte d'autres fichiers invisibles bizarres)
			{
				strcpy(res[compteur], ep->d_name);	//on inscrit le nom du fichier dans le tablo
				compteur++;
			}
		}
		closedir(dp);	//Fermeture du répertoire d'images
...
v2d getArrayOfExemples(vector<char*> tabloFichiers, int nb_exemples, string directory)
{
	v2d res;
	cout << "Lecture des exemples ... " << flush;
	for (int i(0); i < nb_exemples; i++)
		res.push_back(readExemple(tabloFichiers[i], FIRST_LAYER_SIZE, directory));	//on lit chacun des exemples
